...
schema.virtual('firstName').get(function() {
	var firstIndexOfSpace = this.name.indexOf(" ");
	return this.name.substr(0, firstIndexOfSpace);
});
...
schema.virtual('lastName').get(function() {
	var lastIndexOfSpace = this.name.lastIndexOf(" ");
	return this.name.substr(lastIndexOfSpace+1, this.name.length);
});
